// DristiGuide Web Interface JavaScript

class DristiGuideInterface {
    constructor() {
...
function closeEmergencyModal() {
    window.dristiGuideInterface.closeEmergencyModal();
}

function callEmergency() {
    window.dristiGuideInterface.callEmergency();
}

// Initialize the application when DOM is loaded
document.addEventListener('DOMContentLoaded', () => {
    window.dristiGuideInterface = new DristiGuideInterface();
    
    // Add initial welcome message
    window.dristiGuideInterface.addAlert('info', 'System initialized successfully');
    
    // Simulate some initial activity
    setTimeout(() => {
        window.dristiGuideInterface.addAlert('warning', 'Low battery warning: 15% remaining');
    }, 10000);
    
    // Simulate motion detection
    setTimeout(() => {
        window.dristiGuideInterface.addAlert('info', 'Motion detected: User walking');
    }, 15000);
});

// Cleanup on page unload
window.addEventListener('beforeunload', () => {
    if (window.dristiGuideInterface) {
        window.dristiGuideInterface.destroy();
    }
});